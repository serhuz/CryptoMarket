...
class MarketView extends Ui.View {
    ...
    function onUpdate(dc) {
        ...
        if (Dimens.shouldDrawChange) {
            drawPriceChange(dc);
        }
        if (Dimens.shouldDrawPosition) {
            drawPosition(dc);
        }
    ...
    function drawPair(dc) {
        ...
        dc.drawText(dc.getWidth() / 2, Dimens.pairOffset, Gfx.FONT_TINY, text, justification);
    }
    ...
    function drawPriceChange(dc) {
        ...
        if (ticker != null) {
            ...
            dc.drawText(dc.getWidth() / 2, Dimens.priceChangeOffset, Gfx.FONT_TINY, text, justification);
        } else {
            dc.setColor(Gfx.COLOR_LT_GRAY, Gfx.COLOR_BLACK);
            text = "-:-";
            dc.drawText(dc.getWidth() / 2, Dimens.priceChangeOffset, Gfx.FONT_TINY, text, justification);
    ...
    function drawLastPrice(dc) {
        ...
        dc.setColor(Gfx.COLOR_WHITE, Gfx.COLOR_BLACK);
        dc.drawText(dc.getWidth() / 2, dc.getHeight() / 2 - Dimens.lastOffset, Gfx.FONT_MEDIUM, text, justification);
    }
    ...
    function drawAdditionalData(dc) {
        ...
        dc.drawText(dc.getWidth() / 2, dc.getHeight() / 2 + Dimens.askOffset, Gfx.FONT_TINY, text, justification);
        ...
        dc.drawText(dc.getWidth() / 2, dc.getHeight() / 2 + Dimens.bidOffset, Gfx.FONT_TINY, text, justification);
    }
    ...
    function drawPosition(dc) {
        ...
        if (current == null || size == null) {
            dc.drawText(
                dc.getWidth() / 2,
                dc.getHeight() - Dimens.positionOffset,
                Gfx.FONT_XTINY,
                "-/-",
                justification
            );
    ...
    function drawIndicators(dc) {
        ...
        var bottomY = dc.getHeight() - Dimens.indicatorOffset - Dimens.indicatorSize;

        dc.fillPolygon([
             [centerX, dc.getHeight() - Dimens.indicatorOffset],
             [centerX - Dimens.indicatorSize, bottomY],
             [centerX + 5, bottomY]
        ]);

        var topY = Dimens.indicatorOffset + Dimens.indicatorSize;
        dc.fillPolygon([
             [centerX, Dimens.indicatorOffset],
             [centerX - Dimens.indicatorSize, topY],
             [centerX + 5, topY]
        ]);
    }
}
